...
## URGENT NOTE about SHOPIFY'S mandated Authorize.Net Gateway Plugin Change
Authorize.Net has been forced to change its plugin for Shopify, per their [article here](https://support.authorize.net/knowledgebase/Knowledgearticle/?code=KA-04246), and all Shopify stores will be required to install this gateway plugin update by **March 2025**. The issue is how Shopify has changed the requirements (see section titled "What are the known changes or gaps with the new integration?" in the above-linked article). The Authorize.Net plugin can no longer store the actual Authorize.Net transaction ID that links to Authorize.Net. This is a bullying tactic used by Shopify to push store owners to use the Shopify Payments option (and make Shopify more money). We currently have a workaround to this imposed limitation, but it is not part of the current netsuite-authorize.net-connect codebase, which is actively being refined. If you are connecting your Shopify store to NetSuite and expect card processing to continue as it has and need this functionality for now, don't hesitate to contact us. We will discuss your options as we work to solidify a viable workaround within this solution. (updated 27-February-2025)

We currently have a valid workaround for this issue. It requires API access to your Shopify site to write back updates when the order is captured in NetSuite, whereas this used to work. Deploying this solution takes just an hour or two. Contact [us](https://www.gocloud1001.com/contact-us/) for details!
...
- [**A+**] (as of 2024.4.1) "Pay Now" link functionality allows you to send an encrypted URL to your customers and let them pay their invoices without your intervention.  This will allow them to use an existing payment method or enter a whole new method to store for future use!
...
In WebStorm select the machine to machine connection option inside the NetSuite plugin, add the account ID, the Certificate ID and upload the key file (if you are installing into a NetSuite sandbox - and your sandbox ID is like 654321-sb1, the value in the configuration for Instance ID will be 654321-sb1.  Change the "-" to an underscore "_")<br/>      
...
3. If for some reason a Payment Method does not correctly populate, you may need to manually create it and then add it to the configuration.
...
All code is installed in SuiteScripts > openSuite > netsuite-authnet <br/>      
...
## Roadmap
2025.1.x Removal of Ext library references removed by Oracle NetSuite and other UI tweaks as well as general bug fixes that have been reported  
